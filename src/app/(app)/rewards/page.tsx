      ...
      */}
      ...
      {rows.length > 0 ? (
        ...
        >
          ...
          {/*
            インボイス（適格請求書）の注意書き。
            2026-08-27 の打合せで、金額の表示を全部税込にそろえた。
            そのうえで「適格請求書発行事業者でない相手には、契約書のとおり
            消費税相当額を対象外として税抜で精算する」という但し書きを
            画面にも出しておきたい、という庄司さんの依頼。
            契約書と条件表に同じ文言が載っているので、言葉をそろえてある。
          */}
          {showReward ? (
            <div className="border-t border-ink-800 px-5 py-3 text-xs leading-relaxed text-ink-400">
              ※ 表示している金額はすべて税込です。適格請求書発行事業者でない方への
              お支払いは、契約書のとおり消費税相当額をお支払いの対象外とし、
              税抜金額にて精算いたします。
            </div>
          ) : null}
        </Card>

...
export function progressOf(src: ProgressSource): ProgressState {
   ...
   * お金が届いていれば、審査は通っている。
   *
   * アプラスは信販会社の審査に通らないと立替金が入らないので、
   * 「決済完了（着金）なのに審査中」という状態は実際には起こらない。
   * ところが審査結果は受注側で別に持っているため、本部が着金を確認して
   * お支払いを「決済完了」に変えても、審査が「申込中」のままだと
   * 棒が「申込 20%」から動かなかった（2026-08-31）。
   * 変えたことが画面に出ないので、二度押しや「反映されない」の元になる。
   */
  if (paid) {

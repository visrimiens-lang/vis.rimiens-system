...
export async function updateShipmentAction(
  ...
  /*
   * 手渡し・店舗受け取りなど、送り状を使わないお届け（2026-09-07）。
   *
   * 銀座本店で「店舗でそのままお渡しした1台」を出荷済にできなかった。
   * 郵送でないお届けには送り状番号が存在しないので、
   * 画面のチェックで意思表示されたときだけ、番号なしの出荷済を許す。
   */
  const noTracking = text(formData, "noTracking") === "true";
  if (next === "出荷済" && !tracking && !noTracking) {
    return {
      error:
        "出荷済にするには送り状番号が必要です。ヤマトの送り状番号を入力してから、もう一度お試しください。" +
        "まだ送り状ができていない場合は「出荷手配中」で保存してください。" +
        "手渡し・店舗受け取りなど送り状を使わないお届けは、「送り状を使わないお届け」にチェックを入れてください。",
    };
  }

 ...
 *
 * ■ 【銀行振込】【アプラス（ローン）】
 * 支払方法ごとに商品を分けている（2026-08-28）。中身は同じ商品なので、
 * 商品マスタには1本だけ登録し、名前の末尾の但し書きは照合前に外す。
 * この但し書きは受注の決済方法を読むのに使うので（api/webhooks/order）、
 * 商品名からは消さず、ここでの突き合わせのときだけ無視する。
 */

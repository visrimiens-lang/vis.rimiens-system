...
const primaryBtn =
  "whitespace-nowrap inline-flex items-center gap-2 rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-on-gold transition hover:bg-brand-strong disabled:cursor-not-allowed disabled:bg-ink-700 disabled:text-ink-300";
const quietBtn =
  "whitespace-nowrap inline-flex items-center gap-2 rounded-lg border border-ink-700 px-3.5 py-2 text-sm font-medium text-ink-200 transition hover:border-ink-600 hover:text-ink-50 disabled:cursor-not-allowed disabled:text-ink-500";
const dangerBtn =
...
export function QrPanel({ agency }: { agency: QrSource }) {
  ...
  // コンプライアンス対応で止めている最中かどうか。止めている間は発行できない。
  ...
  return (
    <section className="space-y-4 px-5 py-5">
      ...
      {/*
        ...
        2026-08-21 に、研修の合格も本部の承認も条件から外した。
      */}
      {frozen ? (
        <Notice tone="bad">
        ...
        <div className="space-y-4">
          ...
          {!unfreezeOpen ? (
            ...
            <ConfirmReasonForm
              ...
              hint={
                "解除すると、体験のご案内（QR1）とご契約のご案内（QR2）を出し直し、" +
                "そのままご案内のメールをお送りします。" +
                "何を確かめて再開したのかを、必ず残してください。"
              }
              confirmQuestion={(v) =>
                `${who} のQRの停止を解除します（理由：${v}）。ご案内を出し直し、メールでお送りします。よろしいですか？`
              }
        ...
        <div className="space-y-4">
          <div className="rounded-lg border border-ink-800 bg-ink-850/60 px-4 py-3">
            ...
            <ul className="mt-2 space-y-1.5 text-sm leading-relaxed text-ink-300">
              ...
              <li>
                ・解除するまで、新しくお渡しすることもできなくなります。
                解除すると、ご案内を出し直してメールでお送りします。
              </li>
              <li>
